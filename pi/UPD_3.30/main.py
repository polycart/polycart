...
class LeadScreen(DefaultScreen):
    ...
    def search(self):
        ...
        for one in goodslist:
            for i in range(10):
                print(one)
            self.scroll.add('http://39.96.48.80/' + one[1], one[0] + '\n￥' + str(one[2]), (one[3], one[4]))
    def show(self, pos, name):
        self.add_widget(Point((self.the_map.pos[0] + pos[0], self.the_map.pos[1] + pos[1]), name))
    def back(self):
        sm.current = 'Menu'
    pass
