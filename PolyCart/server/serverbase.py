...
class ServerBase(Thread):
	...
	def GetGoodsById(self):
		id = self.clisock.recv(self.bufsiz).decode()
		res = self.goodsdb.select(id)
		self.clisock.send(str(res).encode())
		pass

	def GetAds(self):
		adslist = self.adsdb.getlist()
		self.clisock.send(str(len(adslist)).encode())
		for one in adslist:
			time.sleep(0.1)
			self.clisock.send(str(one).encode())
	
	def RequestMmanualIntervention(self):
		pos = self.clisock.recv(self.bufsiz).decode()
		print(str(pos) + 'NEEDS MANUAL INTERVENTION !!!')
		self.clisock.send("SUCCESSFUL".encode())
			...
			try:
				...
				try:
					if mark == 'WEL':
						...
						self.RequestMmanualIntervention()
					else:
						self.Error(mark)
				except:
					print('logout')
